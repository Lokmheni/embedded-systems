...
void configureGraphics_Sub_Plate()
{
    ...
    // Configure background BG3 in extended rotoscale mode using 8
    // bit pixels and "0" for the base address
    BGCTRL_SUB[ 3 ] = BG_MAP_BASE( 0 ) | BgSize_B8_256x256;


    // Transfer image bitmap and palette to the corresponding
    // memory locations
    dmaCopy( plateBitmap, BG_MAP_RAM_SUB( 0 ), plateBitmapLen );
    dmaCopy( platePal, BG_PALETTE_SUB, platePalLen );
